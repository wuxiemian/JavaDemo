...
public class Bullet {
    ...
    public void move() {
        ...
        if (x > ct.getGameWidth() || y > ct.getGameHeight() || x < 0 || y < 0) {
            ct.bts.remove(this);
        }
    }
    ...
    public boolean hitPlane(Plane p) {
        if (p.getRec().intersects(this.getRec()) && p.isGood() != this.isGood()) {
            ...
            switch (pp) {
                case 0:
                    ct.pps.add(new Props(p.x, p.y, 1, ct));
                    break;
                case 1:
                    ct.pps.add(new Props(p.x, p.y, 1, ct));
                    break;
                case 2:
                    ct.pps.add(new Props(p.x, p.y, 2, ct));
                    break;
                case 3:
                    ct.pps.add(new Props(p.x, p.y, 2, ct));
                    break;
                case 4:
                    ct.pps.add(new Props(p.x, p.y, 3, ct));
                    break;
                case 5:
                    ct.pps.add(new Props(p.x, p.y, 3, ct));
                    break;
                case 6:
                    ct.pps.add(new Props(p.x, p.y, 4, ct));
                    break;
            }
            if (p instanceof Plane1) {
                ct.setScore(ct.getScore() + 5);
            } else if (p instanceof Plane2) {
                ct.setScore(ct.getScore() + 10);
            }
            SoundTool.play(SoundTool.dieSound);
            ct.bts.remove(this);
            return true;
        }
        return false;
    ...
    public boolean hitMyPlane(MyPlane p) {
        if (p.getRec().intersects(this.getRec()) && p.isGood() != this.isGood()) {
            if (R != 20) p.setBlood(p.getBlood() - 5);
            else p.setBlood(p.getBlood() - 10);
            ct.bts.remove(this);
            return true;
        }
        return false;
    ...
    public boolean hitBOSS(BOSS b) {
        if (this.isGood() && b.getRec().intersects(this.getRec())) {
            if (b.getBlood() > 0) {
                ...
                SoundTool.play(SoundTool.bossDieSound);
                b.setLive(false);
